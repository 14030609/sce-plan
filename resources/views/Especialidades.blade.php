...
<div class="container ">
    ...
    <div class="table-responsive text-center">
        <table class="table table-borderless" id="table">
            ...
            <tr class="item{{$item->cveesp}}">
                <td>{{$item->cveesp}}</td>
                <td>{{$item->descripcion}}</td>
                <td>
                    <button class="edit-modal btn btn-info"
                            data-info="{{$item->cveesp}},{{$item->descripcion}}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{ Form::open(array('action' => 'EspecialidadesController@updateDos', 'method' => 'post')) }}
                <div class="form-group">
                    <label class="control-label col-sm-2" for="CveEsp">cveEspecilidad:</label>
                    <div class="col-sm-10">
                        {{ Form::text('cveesp', '', array('id' => 'cveesp_show', 'class' => 'form-control input-sm', 'placeholder' => 'clave de taller', 'required' => 'required')) }}
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        ...
                        {{ Form::text('descripcion','', array('id' => 'descripcion_show', 'class' => 'form-control input-sm', 'placeholder' => 'Taller', 'required' => 'required')) }}
                    </div>
                </div>
...
<script>
    ...
    $(document).on('click', '.add-modal', function() {
        ...
        $('#cveesp_show').val($(this).data('cveesp'));
    ...
    function fillmodalData(details){
        $('#fid').val(details[0]);
        $('#descripcion').val(details[1]);
    }

    $('.modal-footer').on('click', '.edit', function() {
        $.ajax({
            type: 'post',
            url: '/editEspecialidad',
            data: {
